...
//APP生命周期中(类初始化) 只会执行一次
+ (void)initialize
{
    // 若调用是自己(排查子类调用) mark byKing
    if (self == [RootNavigationController self]) {
        ...
        [navBar setBarTintColor:CNavBgColor]; // 背景色
        [navBar setTintColor:CNavBgFontColor]; // 栏按钮渲染颜色
...
-(void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    else if ([viewController isKindOfClass:[RootTableViewController class]]) {
        RootTableViewController * vc = (RootTableViewController *)viewController;
        if (vc.isHidenNaviBar) {
            vc.view.top = 0;
            [vc.navigationController setNavigationBarHidden:YES animated:animated];
        }else{
