...
//播放的语音数组
@property (nonatomic, copy) NSArray <SRXMsgChatModel *>*audioArr;
//定时器
...
- (void)preparePlayAudio
{
    ...
    if (fileName.length > 0)
    {
        ...
        if ([fileName isValidUrl]) {
            [self startPlayAudioWithUrl:fileName];
        }else {
            //拼接WAV文件路径
...
//            }
            //获取AAC文件路径
            NSString *aacfile = [SHFileHelper getFilePathWithName:fileName type:SHMessageFileType_aac];
            //开始播放
            [self startPlayAudioWithWavFile:aacfile];
        }
...
{
    SRXMsgChatModel *message = self.audioArr[_playingIndex];
    self.playMark = message.voice;
...
- (void)startPlayAudioWithUrl:(NSString *)url
{
    SRXMsgChatModel *message = self.audioArr[_playingIndex];
    self.playMark = message.params;
...
- (NSString *)getAudioFileName
{
    if (self.audioArr.count > _playingIndex && _playingIndex != -1)
    {
        ...
        return message.params?:message.voice;
    }
    _playingIndex = -1;
    return @"";
}
