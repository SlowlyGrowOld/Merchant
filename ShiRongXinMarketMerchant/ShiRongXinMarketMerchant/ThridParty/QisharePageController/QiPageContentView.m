...
- (void)setPageContentShouldScrollToIndex:(NSInteger)index beforIndex:(NSInteger)beforeIndex{
    ...
    if (beforeIndex < index) {
        [weakPage.pageViewController setViewControllers:@[weakPage.controllerArray[index]] direction:UIPageViewControllerNavigationDirectionForward animated:weakPage.canAnimated completion:^(BOOL finished) {
            if (finished) {
                weakPage.beforeIndex = index;
                dispatch_async(dispatch_get_main_queue(), ^{
                    [weakPage.pageViewController setViewControllers:@[weakPage.controllerArray[index]] direction:UIPageViewControllerNavigationDirectionForward animated:weakPage.canAnimated completion:nil];
                });
                
            }
        }];
    }else{
        [weakPage.pageViewController setViewControllers:@[weakPage.controllerArray[index]] direction:UIPageViewControllerNavigationDirectionReverse animated:weakPage.canAnimated completion:^(BOOL finished) {
            if (finished) {
                 weakPage.beforeIndex = index;
                dispatch_async(dispatch_get_main_queue(), ^{
                    [weakPage.pageViewController setViewControllers:@[weakPage.controllerArray[index]] direction:UIPageViewControllerNavigationDirectionReverse animated:weakPage.canAnimated completion:nil];
                });
...
- (void)setCanAnimated:(BOOL)canAnimated {
    _canAnimated = canAnimated;
}
...
- (UIPageViewController*)pageViewController {
    if (!_pageViewController) {
        ...
        if (self.controllerArray.count) {
            [_pageViewController setViewControllers:@[self.controllerArray[0]] direction:UIPageViewControllerNavigationDirectionForward animated:self.canAnimated completion:^(BOOL finished) {
            }];
        }
    }
...
- (void)setControllerArray:(NSArray *)controllerArray {
    ...
    if (_controllerArray.count) {
        _beforeIndex = 0;
        [self.pageViewController setViewControllers:@[self.controllerArray[0]] direction:UIPageViewControllerNavigationDirectionForward animated:self.canAnimated completion:^(BOOL finished) {
        }];
    }
}
