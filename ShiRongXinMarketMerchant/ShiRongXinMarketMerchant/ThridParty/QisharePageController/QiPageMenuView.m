...
/**
 调用- (void)updateMenuViewWithNewItemArray:(NSArray *)items selectedIndex:(NSInteger)selectedIndex;时,滑动禁止动画
 */
@property (nonatomic,assign)CGFloat isAnimated;
@end
...
- (void)scrollToPageItem:(QiPageItem*)pageItem {
    ...
    if (CGRectGetMidX(convertRect) < CGRectGetMidX(self.frame)) {
        ...
        [self setContentOffset:CGPointMake(targetX, 0) animated:self.isAnimated];
