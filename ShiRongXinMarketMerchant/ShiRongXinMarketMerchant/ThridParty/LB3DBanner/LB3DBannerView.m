...
-(void)setUpImageViews
{
    ...
    float midWidth = 245;
    float midHeight = height-offsetY / 2;
    float midX = (width-midWidth)/2;
    float midY = offsetY / 2;
    midRect = CGRectMake(midX, midY, midWidth, midHeight);
    
    float leftWidth = midWidth;
    float leftHeight = midHeight;
    float leftX = -leftWidth/2;
    float leftY = 0;
    leftRect = CGRectMake(leftX, leftY, leftWidth, leftHeight);
    
    float rightWidth = leftWidth;
    float rightHeight = leftHeight;
    float rightX = width - rightWidth/2;
    float rightY = 0;
...
//    NSLog(@"%@",view.imageURL);
    ...
    {
        [self.delegate didTapTheMidImageView:self.currentIndex];
    }
}
...
-(void)scrollToDirection:(SwipeDirection)swipeDirection
{
    switch (swipeDirection)
    {
        ...
        case SwipeLeft:
        {
            ...
            {
                if (self.currentIndex + 1 >= self.imageURLArr.count) {
                    self.rightBGImageView.imageURL = @"";
                }else {
            ...
            {
                if (self.imageURLArr.count > self.currentIndex +2) {
                    self.leftBGImageView.imageURL = self.imageURLArr[self.currentIndex + 2];
                }else {
                    self.leftBGImageView.imageURL = @"";
                }
            }
