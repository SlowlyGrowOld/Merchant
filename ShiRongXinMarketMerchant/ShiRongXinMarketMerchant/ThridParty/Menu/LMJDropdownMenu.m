...
- (void)layoutSubviews {
    ...
    [_optionsList setFrame:CGRectMake(0, height+4, width, _optionsList.frame.size.height)];
}
...
- (void)initProperties{
    ...
    _optionBgColor          = KWhiteColor;
    _optionFont             = [UIFont systemFontOfSize:12];
    _optionTextColor        = UIColorHex(#666666);
...
- (void)initViews{
    ...
    _optionsList.layer.cornerRadius = 8;
    _optionsList.layer.masksToBounds = YES;
...
- (void)showDropDown{   /* 显示下拉列表 */
    ...
    [UIView animateWithDuration:self.animateTime animations:^{
        ...
        weakSelf.arrowMark.transform = CGAffineTransformMakeRotation(weakSelf.rotateEnabled?M_PI:0);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    titleLabel.frame = CGRectMake(self.optionTextMarginLeft, 0, self.frame.size.width - self.optionTextMarginLeft - self.optionIconMarginRight, cHeight);
    
    UIImageView * icon = [cell viewWithTag:888];
    if ([self.dataSource respondsToSelector:@selector(dropdownMenu:iconForOptionAtIndex:)]){
        icon.frame = CGRectMake((self.frame.size.width -self.optionIconSize.width)/2, (cHeight - self.optionIconSize.height)/2, self.optionIconSize.width, self.optionIconSize.height);
